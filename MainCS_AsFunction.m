...
for m = 1:length(all_files)
    ...
    for n = 1:length(successful_files)
        name2 = successful_files(n);
        if contains(name2, temp_name)
            case_was_successful = 1;
            break
        end
    end
    ...
    if (bone_indx == 3 || bone_indx == 5 || bone_indx == 6 || bone_indx == 13 || bone_indx == 14)
        ...
        if bone_indx == 1
            bone_coord = [1,2,3]; %all talus coordinate systems
...
% (max_Z > crit_Z && (bone_indx ~= 1 || 14))
        if plot_different == 1 && ((max_Z > crit_Z) || (plotted_good == 0 && max_Z <= crit_Z))
            ...
            if max_Z > crit_Z
                title(strcat('Coordinate System of'," ", char(FileName), " is maybe different"),'Interpreter','none')
            else
                title(strcat('Coordinate System of'," ", char(FileName), " is similar"),'Interpreter','none')
            end
